...
struct Params {
    ...
    // Weak-reflection threshold, as an ORBIT TARGET INTENSITY. An orbit whose
    // target falls below this keeps its CALCULATED amplitude and has its phase
    // advanced by pi/2 instead of being forced onto |Fobs| (Oszlanyi & Suto
    // 2005). Zero disables the branch entirely, reproducing the 2004 method,
    // and is what the host writes unless job.weakFraction says otherwise.
    weak_i       : f32,
    _pad1        : u32,
    _pad2        : u32,
};
...
fn constrain(@builtin(global_invocation_id) gid3 : vec3<u32>) {
    ...
    if (P.weak_i > 0.0 && tgt < P.weak_i) {
        // Weak: keep |Fcalc|, rotate the phase by +pi/2. For a member holding
        // the Friedel mate the stored value is conj(F), and conj(i*F) is
        // -i*conj(F), so the rotation has the opposite sense there.
        for (var i = 0u; i < orb.count; i = i + 1u) {
            let p    = orb.start + i;
            let word = orbitIdx[p];
            let idx  = word & IDX_MASK;
