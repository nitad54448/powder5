...
// version 164, 19 august 2026 -- GPU (WebGPU) path with CPU fallback,
// space group applied inside the iteration.
//
// Dual-space charge flipping (Oszlanyi & Suto, Acta Cryst A60 (2004) 134)
// run on Pawley-extracted integrated intensities. The modified code by Oszlany in 2005 is implemented since v 161
// via a slider in the CF panlel, default 15%
// ---------------------------------------------------------------------------
...
if (typeof importScripts === 'function') {
    // Charge flipping only. The Wyckoff modules (symmetry_utils, observations,
    // wyckoff_assign, scatterers, contacts, swarm_wyckoff, coord_refine) moved
    // to wyckoff_worker.js with the search that used them, so this worker no
    // longer pays to parse them.
...
// ---------------------------------------------------------------------------
//  THE WYCKOFF SEARCH USED TO LIVE HERE. IT NOW LIVES IN js/wyckoff_worker.js.
//
//  It never used a density map, never used the flipping kernel, and shared
//  nothing with this file but the file itself -- which was enough to make a
//  Wyckoff arithmetic error report itself as "charge_flipping_worker.js:2782"
//  and read as a coupling between two methods that are independent.
//
//  Moved with it: CF_ATOM_DATA, cfRawObservationRows, cfObservationsFromHkl,
//  runWyckoffFromIntensities and the 'wyckoff-search' / 'cancel-wyckoff'
//  messages. Nothing in this file referenced any of them.
// ---------------------------------------------------------------------------
