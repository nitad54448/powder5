...
const Density3D = {
    ...
    init(canvas) {
        ...
        // without the far side going pure black.
        ...
        this._three = {
            THREE, canvas, renderer, scene, camera, root,
            ...
            theta: 0.9, phi: 1.05, radius: 3, target: new THREE.Vector3(),
            needsRender: true
        };
    ...
    _draw() {
        ...
        t.camera.position.set(
            t.target.x + r * Math.sin(t.phi) * Math.sin(t.theta),
            t.target.y + r * Math.cos(t.phi),
            t.target.z + r * Math.sin(t.phi) * Math.cos(t.theta)
        );
    ...
    show(opts) {
        ...
        if (showCell !== false) {
            ...
            const AXIS_COLORS = [[0.95, 0.30, 0.28], [0.42, 0.85, 0.38], [0.36, 0.62, 0.98]];
            const GREY = [0.44, 0.47, 0.51];
    ...
    resetView() {
        const t = this._three;
        if (!t) return;
        t.theta = 0.9; t.phi = 1.05; t.radius = 3;
        t.needsRender = true;
    },
